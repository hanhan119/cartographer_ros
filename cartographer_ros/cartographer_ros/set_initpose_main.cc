...
void move_base_simple_callback(
  ...
  {
      std::cout<<"\e[9m\e[37m\e[44m  current_trajectory_id :"<<current_trajectory_id<<"\033[0m"<<std::endl;
  }
  ...
  std::cout<<"\e[9m\e[37m\e[44m  srv_finish_trajectroy.request.trajectory_id :"<<srv_finish_trajectroy.request.trajectory_id<<"\033[0m"<<std::endl;
...
//   tf2::toMsg(relative_initpose_tf, srv_start_trajectroy.request.initial_pose);
  srv_start_trajectroy.request.initial_pose = msg->pose.pose;
...
//   map_builder_->LoadState(&reader, true);
  ...
  ::ros::init(argc, argv, "cartographer_set_initpose");
  ::ros::start();
